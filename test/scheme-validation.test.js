...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // 3.x has no mailto scheme handler (it arrived in v4), so the ws handler
  // stands in for it: it only fills in `resourceName` when its parser runs,
  // which requires the scheme to be decoded before the handler is looked up.
  const ws = fastURI.parse('w%73://example.com/chat')
  t.equal(ws.scheme, 'ws', 'the scheme is decoded before the handler is selected')
